...
void setup() {
  ...
  for(int i=0 ;i< Canvas_height(); ++i) {
      for(int t=0; t<Canvas_width();++t) {
        *( (uint16_t*) RGB565Canvas + t + (i*Canvas_width()) )=(uint16_t) (i*t/2); // fill screen with pattern
       }
  }
  tt=millis()-tt;
  ...
  Canvas_FillRect(700,20,10,300,RED);
  Canvas_FillRect(20,30,10,440,MAGENTA);
  ...
  Invaders3_1.ypos = random(0, Canvas_height()/2);
...
while(1) {
      tx=micros();
      for(int t=0;t<240;++t){
      xp+=15 ; if (xp>Sprite_width()) xp-=Sprite_width();//random(Canvas_height()/2,Canvas_height()-INVADER_Y+1);}  
      Invaders1_1.xpos = xp;Invaders2_1.xpos=xp+40;Invaders3_1.xpos=xp+80;Invaders4_1.xpos=xp+100;
      yp1+= random(0,8); if (yp1 > Sprite_height())  yp1 -=Sprite_height();
      yp2+= random(0,12); if (yp2 > Sprite_height()) yp2 -=Sprite_height();
      yp3+= random(3,7); if (yp3 > Sprite_height())  yp3 -=Sprite_height();
      yp4+= random(5,10); if (yp4 > Sprite_height())  yp4 -=Sprite_height();
